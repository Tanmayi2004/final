...
app.post('/bookappointment', async function (req, res) {
    try {
        ...
        // Assuming you have a field like 'doctorEmail' to match appointments with a specific doctor
        const data = await appointments.find({ emailid: req.body.emailid }).toArray();
        conn.close();
        res.json(data);
    } catch (err) {
        res.json(err).status(404);
...
app.post('/sendemail', async function(req, res){
    try
    {
        var transport = nodemailer.createTransport({
            ...
            port: 445,
            secure: true,
            auth:{user: "tanmayimovva30@gmail.com", pass: "ohksndgfstcoapyj"}
        });

        var emaildata = {
            from: "tanmayimovva30@gmail.com",
            to: "t50682717@gmail.com",
